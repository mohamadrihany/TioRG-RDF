...
public class JenaJungGraph implements DirectedGraph<RDFNode, Statement> {
    ...
    public Collection<Statement> getOutEdges(RDFNode vertex) {
        if (log) {
            System.err.println("getOutEdges");
        }
        if (vertex.isLiteral()) {
            return Collections.EMPTY_LIST;
        } else {
            return asCollection(model.listStatements((Resource) vertex, null, (RDFNode) null));
        }
    }

    public Collection<RDFNode> getPredecessors(RDFNode vertex) {
        if (log) {
            System.err.println("getPredecessors");
        }
    ...
    public Collection<RDFNode> getSuccessors(RDFNode vertex) {
        if (log) {
            System.err.println("getSucessors");
        }
        if (vertex.isLiteral()) {
            return Collections.EMPTY_LIST;
        } else {
            return asCollection(model.listObjectsOfProperty((Resource) vertex, null));
        }
    }
    ...
    public boolean isPredecessor(RDFNode v1, RDFNode v2) {
        if (log) {
            System.err.println("isPredecessor");
        }
        if (v1.isLiteral()) {
            return false;
        } else {
            return model.contains((Resource) v1, null, v2);
        }
    }

    public boolean isSuccessor(RDFNode v1, RDFNode v2) {
        if (log) {
            System.err.println("isSuccessor");
        }
    ...
    public int getPredecessorCount(RDFNode vertex) {
        if (log) {
            System.err.println("getPredecessorCount");
        }
    ...
    public int getSuccessorCount(RDFNode vertex) {
        if (log) {
            System.err.println("getSucessorCount");
        }
    ...
    public Pair<RDFNode> getEndpoints(Statement edge) {
        if (log) {
            System.err.println("getEndpoints: " + edge);
        }
    ...
    public RDFNode getOpposite(RDFNode vertex, Statement edge) {
        if (log) {
            System.err.println("getOpposite");
        }
        if (edge.getSubject().equals(vertex)) {
            return edge.getObject();
        } else {
            return edge.getSubject();
        }
    }
    ...
    public Collection<RDFNode> getVertices() {
        if (log) {
            System.err.println("getVertices");
        }
    ...
    public boolean containsVertex(RDFNode vertex) {
        if (log) {
            System.err.println("containsVertex");
        }
        if (vertex.isResource()
                && model.contains((Resource) vertex, null, (RDFNode) null)) {
            return true;
        }
    ...
    public boolean containsEdge(Statement edge) {
        if (log) {
            System.err.println("containsEdge");
        }
    ...
    public int getEdgeCount() {
        if (log) {
            System.err.println("getEdgeCount");
        }
    ...
    public int getVertexCount() {
        if (log) {
            System.err.println("getVertexCount");
        }
    ...
    public Collection<RDFNode> getNeighbors(RDFNode vertex) {
        if (log) {
            System.err.print("getNeighbours");
        }
    ...
    public Collection<Statement> getIncidentEdges(RDFNode vertex) {
        if (log) {
            System.err.println("getIncidentEdges");
        }
    ...
    public Collection<RDFNode> getIncidentVertices(Statement edge) {
        if (log) {
            System.err.println("getIncidentVertices");
        }
    ...
    public Statement findEdge(RDFNode v1, RDFNode v2) {
        if (log) {
            System.err.println("findEdgeSet");
        }
    ...
    public boolean isNeighbor(RDFNode v1, RDFNode v2) {
        if (log) {
            System.err.println("isNeighbour");
        }
    ...
    public EdgeType getEdgeType(Statement edge) {
        if (log) {
            System.err.println("getEdgeType");
        }
    ...
    public EdgeType getDefaultEdgeType() {
        if (log) {
            System.err.println("getDefaultEdgeType");
        }
        return EdgeType.DIRECTED;
    }
    ...
    public Collection<Statement> getEdges(EdgeType edge_type) {
        if (log) {
            System.err.println("getEdges(type)");
        }
        if (edge_type.equals(EdgeType.DIRECTED)) {
            return getEdges();
        } else {
            return Collections.EMPTY_LIST;
        }
    ...
    public int getEdgeCount(EdgeType edge_type) {
        if (log) {
            System.err.println("getEdgeCount(type)");
        }
        if (edge_type.equals(EdgeType.DIRECTED)) {
            return (int) model.size();
        } else {
