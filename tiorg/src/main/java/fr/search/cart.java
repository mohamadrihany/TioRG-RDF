...
public class cart {
     ...
     * @param sets
     */
    public static  ArrayList<ArrayList<ModelNode>> Cart(ArrayList<ArrayList<ModelNode>> sets2) {
    ArrayList<ArrayList<ModelNode>> cartisienProductResult=new  ArrayList<ArrayList<ModelNode>>();
   ArrayList<ArrayList<ModelNode>> sets=new ArrayList<>();
    for(ArrayList<ModelNode> set:sets2){
        if(!set.isEmpty())
        {
            sets.add(set);
        }
    ...
    for(int i = 0; i < solutions; i++) {
        ...
        int j = 1;
        for(ArrayList<ModelNode> set : sets) {
