...
import fr.uvsq.adam.processings.GetEquivalentsFromXMLFile;
...
public class UsingPatternsEdges {
    ...
    public static ArrayList<Model> main(String uri, String type, String content, String keyword, HashMap<String, SearchElements.KeywordsReleventFragment> releventElementList, Model model, Element racine) throws Exception {
...
//la liste des noeuds d'extremite
        ArrayList<UsingPatternsNodes.TypeReleventFragment> nodesListeToUse = new ArrayList<>();

        while (it.hasNext()) {
            String keyword2 = it.next();

            if (!keyword2.equals(keyword)) {
                ArrayList<SearchElements.ElementReleventFragmentNodes> nodesList = releventElementList.get(keyword2).ReleventFragmentNodes;

                for (SearchElements.ElementReleventFragmentNodes list : nodesList) {
                    ArrayList<UsingPatternsNodes.TypeReleventFragment> releventFragement = list.ReleventFragment;
                    ArrayList<UsingPatternsNodes.TypeReleventFragment> releventFragementModel = list.ReleventFragmentModel;

                    for (UsingPatternsNodes.TypeReleventFragment relevent : releventFragement) {
                        nodesListeToUse.add(relevent);
                    }
                    for (UsingPatternsNodes.TypeReleventFragment relevent : releventFragementModel) {
                        nodesListeToUse.add(relevent);
                    }
        ...
        for (UsingPatternsNodes.TypeReleventFragment extremity : nodesListeToUse) {
