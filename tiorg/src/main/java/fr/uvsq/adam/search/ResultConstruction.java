...
public class ResultConstruction {
	...
	public static ArrayList<ResultNodeModel> main(ArrayList<ArrayList<SetSetReleventFragments.ModelNode>> cartesienProduct,
			...
			DirectedGraph<RDFNode,Statement> graph)
	{
		...
		for(ArrayList<SetSetReleventFragments.ModelNode> modelNodes : cartesienProduct)
		{
			...
			if( !modelNodes.isEmpty() )
			{
				if(modelNodes.size() > 1)
				{
					...
					SetSetReleventFragments.ModelNode firstModelNode = modelNodes.get(0);
					RDFNode firstNode = firstModelNode.Node;
					for(int indice = 1; indice < modelNodes.size(); indice++)
					{
						SetSetReleventFragments.ModelNode secondModelNode = modelNodes.get(indice);
